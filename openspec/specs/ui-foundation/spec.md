...
### Requirement: Compact timeline block styling

The system SHALL provide compact readable styling for the run-history timeline block in menu surfaces.

#### Scenario: Timeline block remains readable on narrow portrait layouts
- **WHEN** menu is rendered on narrow portrait viewports
...
#### Scenario: Focus and touch affordances are visible
- **WHEN** interactive controls are rendered in menu/run overlays
- **THEN** focus-visible state SHALL be clearly visible
- **AND** touch-oriented controls SHALL meet a minimum target size floor

#### Scenario: Reduced-motion preference minimizes non-essential motion
- **WHEN** reduced motion accessibility state is enabled
- **THEN** non-essential transitions and decorative motion SHALL be reduced or disabled

### Requirement: Action buttons consume shared touch-target token

The system SHALL enforce this contract as part of the znake-ui-action-button-target-token-v1 scope.
...
The system SHALL enforce this contract as part of the znake-ui-status-chip-factory-variants-v1 scope.
...
### Requirement: DOM factory exposes shared attribute application helper

The system SHALL enforce this contract as part of the znake-refactor-dom-factory-attrs-helper-v1 scope.
...
### Requirement: Accessibility media sync avoids duplicate listeners

The system SHALL enforce this contract as part of the znake-refactor-accessibility-media-sync-guard-v1 scope.
...
### Requirement: UI token names stay domain-prefixed

The system SHALL enforce this contract as part of the znake-ui-token-naming-hygiene-v1 scope.
...
### Requirement: Body scene shell class toggles remain mutually exclusive

The system SHALL enforce this contract as part of the znake-ui-shell-class-contract-v1 scope.
...
### Requirement: Static DOM translation map keeps explicit id-key mapping

The system SHALL enforce this contract as part of the znake-i18n-static-dom-map-contract-v1 scope.
...
### Requirement: Interactive CSS classes reuse shared touch-target token

The system SHALL enforce this contract as part of the znake-ui-css-touch-target-token-propagation-v1 scope.
...
### Requirement: Launch card spacing uses shared spacing tokens

The system SHALL enforce this contract as part of the znake-style-launch-card-spacing-tokenization-v1 scope.
