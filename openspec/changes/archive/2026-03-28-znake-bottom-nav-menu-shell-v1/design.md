...
Bottom Nav Menu Shell is proposed as a focused change in the visual track. The goal is to keep gameplay clarity and architecture quality aligned while enabling fast iteration.

## Key Points (Codex-style)

- What is changing
  - Redesign menu IA with a bottom navigation bar for Play, Progress, Settings, and Accessibility.
- Why we are doing it
  - Fixes current menu condensation and improves discoverability by section.
- Impacted areas
  - Menu scene layout, interaction flow, responsive CSS.
- Risks / unknowns
  - Tab switching could hide critical actions if labels are weak.
...
- [Risk] Tab switching could hide critical actions if labels are weak.
- [Trade-off] More granular changes increase coordination overhead but improve rollback safety.

## Migration Plan

1. Finalize spec deltas and tasks for Bottom Nav Menu Shell.
...
Checked 151 files in 108ms. No fixes applied.
...
✓ built in 2.17s when behavior/architecture changes.
